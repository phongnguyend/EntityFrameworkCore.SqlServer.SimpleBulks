...
public class BulkInsertBuilder<T>
{
    ...
    public void Execute(IReadOnlyCollection<T> data)
    {
        ...
        mergeStatementBuilder.AppendLine($"INSERT ({_table.CreateDbColumnNames(_columnNames, includeDiscriminator: true)})");
        mergeStatementBuilder.AppendLine($"VALUES ({_table.CreateColumnNames(_columnNames, "Src", includeDiscriminator: true)})");
        mergeStatementBuilder.AppendLine($"OUTPUT inserted.[{_table.GetDbColumnName(_outputIdColumn)}], Src.[{Constants.AutoGeneratedIndexNumberColumn}]");
        mergeStatementBuilder.AppendLine(";");
    ...
    public void SingleInsert(T dataToInsert)
    {
        ...
        LogParameters(_table.CreateSqlParameters(insertCommand, dataToInsert, columnsToInsert, includeDiscriminator: true, autoAdd: true));
